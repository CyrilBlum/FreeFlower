...
Jede natürliche Zahl \(n \geq 2\) lässt sich als Produkt von endlich vielen Primzahlen darstellen. Ordnet man diese Primzahlen der Grösse nach, so ist diese Darstellung sogar eindeutig.
...
\begin{myexcursion}\label{myexcursion:finiteN}
	...
	\begin{align}
		\text{Prim}(n) & \geq \frac{n}{\ln(n) - 1}, \quad\text{für } n\geq \num{5393},    \\
		\text{Prim}(n) & \leq \frac{n}{\ln(n) - 1.1}, \quad\text{für } n\geq \num{60184}.
	\end{align}
\end{myexcursion}
...
\begin{myexercise}
	\begin{aenum}
		\item Bestimme $\text{Prim}(n)$ für $n = \num{10000000}$ mithilfe der Funktion \lstinline|prim| (aus \cref{myexercise:implementVerbesserung1} oder \cref{myexercise:implementVerbesserung2}) und vergleiche den Wert mit der Schätzung $n/\ln\lr{n}$.
	...
	\begin{myanswer}
		\begin{aenum}
			\item Das Python-Programm liefert $\text{Prim}(10000000) = \num{664579}$. Die Schätzung liefert $10^7/\ln{(10^7)}\approx \num{620421}$. Der relative Fehler ist etwa $6.645$ Prozent.
...
\begin{myexercise}
	Bestimmen Sie eine sinnvolle obere Schranke für den Kommunikationsaufwand $A$, falls $n:=10^{16}$.
	\begin{myanswer}
		\begin{align*}
			A \leq                             &                           \\
			4\ceil{\log_2\lr{n}}               & =                         \\
...
\begin{aenum}
	...
	\end{align*}
	Das Protokoll entscheidet also (unter der Voraussetzung $x \neq y$) genau dann falsch, wenn die Primzahl $p$ die Zahl
	\begin{align*}
	...
	\end{align}
	Gemäss dem Fundamentalsatz der Arithmetik (Primfaktorzerlegung), können wir $w\geq 2$ auf eindeutige Weise\footnote{In \cref{myexercise:wgreater1} erklären Sie, warum $w\geq 2$ gilt.} als $w = p_1^{i_1}p_2^{i_2}\ldots p_k^{i_k}$ darstellen, wobei $p_1<p_2<\ldots<p_k$ Primzahlen  und $i_1, i_2,\ldots ,i_k$ positive ganze Zahlen sind. Unser Ziel ist zu beweisen, dass $k\leq n-1$ ist. Dass $w$ also höchstens $n-1$ verschiedene Primfaktoren besitzt.
	\begin{myproof}
		Angenommen die Aussage wäre falsch, also $k\geq n$. Dann gilt
		\begin{align*}
			w
			= p_1^{i_1}p_2^{i_2}\ldots p_k^{i_k}
			\geq p_1p_2\ldots p_n
			\geq 2\cdot3\cdot4\cdot\ldots\cdot(n+1)
			> 2^n.
		\end{align*}
		Dies ist aber ein Widerspruch zur Tatsache, dass $w<2^n$.
		Damit ist unsere Annahme falsch und $w$ kann nicht mehr als $n-1$
		unterschiedliche Primfaktoren haben.
	\end{myproof}
