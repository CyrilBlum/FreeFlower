...
\begin{figure}[H]
	\centering
	\resizebox{0.45\linewidth}{!}{\input{Grundlagen_Info/03_Kryptologie/Figures/tikz_public_key_encryption}}
...
Bei einer reinen Signatur wird die Nachricht $m$ \textbf{im Klartext} zusammen mit der Signatur $s$ übertragen (siehe \cref{fig:digital-signature-detail}):
\begin{enumerate}
	\item Alice signiert den Klartext $m$ mit ihrem privaten Schlüssel $d_A$: $s = m^{d_A} \bmod n_A$.
	\item Über den Kanal wird das unverschlüsselte Paket $(m, s)$ (Klartext $m$ und Signatur $s$) übertragen.
...
\begin{figure}[H]
	\centering
	\input{Grundlagen_Info/03_Kryptologie/Figures/tikz_digital_signature}
	\caption{Detaillierter Ablauf einer reinen digitalen Signatur (Klartextübertragung mit Authentizitätsnachweis)}
	\label{fig:digital-signature-detail}
\end{figure}
...
\begin{figure}[H]
	\centering
	\input{Grundlagen_Info/03_Kryptologie/Figures/tikz_sign_then_encrypt}
...
\begin{myexercise}[Kombinierte Signatur und Verschlüsselung in der Praxis]
	Alice möchte die geheime Botschaft $m = 4$ an Bob senden. Die Übertragung soll sowohl \textbf{vertraulich} (geheim) als auch \textbf{authentisch} (nachweisbar von Alice) sein. Dazu nutzen beide das \gls{RSA}-Verfahren für \emph{Sign-then-Encrypt}. Führen Sie alle Berechnungen in Python durch (z.\,B. auf \url{https://webtigerpython.ethz.ch}).
	...
	\begin{myanswer}
		\begin{enumerate}
			\item \textbf{Signaturerstellung (mit $d_A = 7$ und $n_A = 33$):}
			      \[
				      s = m^{d_A} \bmod n_A = 4^7 \bmod 33 = 16.
			      \]
		...
		\lstinputlisting{Grundlagen_Info/03_Kryptologie/Python package/encryption/rsa_sign_then_encrypt.py}
	\end{myanswer}
\end{myexercise}