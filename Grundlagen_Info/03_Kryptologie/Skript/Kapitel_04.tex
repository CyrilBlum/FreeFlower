...
\begin{myexcursion}[Warum funktioniert die \texorpdfstring{\acrshort{RSA}}{RSA}-Entschlüsselung? (Mathematischer Beweis)]\label{exc:rsa-beweis}
	Warum stellt die Entschlüsselung $(m^e \bmod n)^d \bmod n$ garantiert die ursprüngliche Nachricht $m$ wieder her?
	
	Der Beweis lässt sich schrittweise nachvollziehen:
	\begin{align*}
		\text{Entschlüsselter Text} 
		  & = c^d \bmod n                                          && (\text{Definition}) \\[0.4em]
		  & = (m^e \bmod n)^d \bmod n                              && (\text{Kryptotext } c = m^e \bmod n) \\[0.4em]
		  & \equiv (m^e)^d \pmod n                                 && (\text{Zwischenreduktion}) \\[0.4em]
		  & \equiv m^{e \cdot d} \pmod n                           && ((a^b)^c = a^{b \cdot c}) \\[0.4em]
		  & \equiv m^{k \cdot \varphi(n) + 1} \pmod n              && (e \cdot d = k \cdot \varphi(n) + 1) \\[0.4em]
		  & \equiv \left(m^{\varphi(n)}\right)^k \cdot m^1 \pmod n && (a^{x+y} = a^x \cdot a^y) \\[0.4em]
	...
	\textbf{Zusammenfassung der Kernidee:} Durch die Wahl von $e \cdot d = k \cdot \varphi(n) + 1$ durchläuft der Exponent genau $k$ volle Umläufe modulo $n$, die nach dem Satz von Euler jeweils neutral zu $1$ werden ($1^k = 1$). Es bleibt exakt der Faktor $m^1 = m$ übrig.
\end{myexcursion}
