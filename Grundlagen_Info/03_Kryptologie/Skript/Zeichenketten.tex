...
\cref{tab:string-op} enthält eine Übersicht einiger nützlicher Befehle, die Sie in diesem Kapitel verwenden werden.

\begin{table}[H]
    \centering
    \begin{longtable}{l|m{0.3\linewidth}|m{0.4\linewidth}}
        \textbf{Python}                    & \textbf{Beschreibung}                                                                                                                          & \textbf{Beispiel}                                                 \\\midrule\midrule
        \lstinline|len(s)|                 & Gibt die Länge des Textes \lstinline|s| zurück.                                                                                                & \begin{lstlisting}[numbers=none]
n = len("hallo") # 5
\end{lstlisting}                                   \\\midrule\midrule
        \lstinline|s[index]|               & Greift auf das Zeichen an der Index-Position \lstinline|index| zu. Bei negativen Indizes zählt man von rechts.                                 & \begin{lstlisting}[numbers=none]
s = "hallo"
s[0]  # "h"
s[1]  # "a"
s[-1] # "o"
\end{lstlisting}                                   \\\midrule
        \lstinline|s[start:end]|           & Extrahiert den Teiltext vom Index \lstinline|start| bis und ohne \lstinline|end|                                                               & \begin{lstlisting}
text = "hallo"
text[1:4] # "all"
\end{lstlisting}                                                 \\\midrule\midrule
        \lstinline|ord(c)|                 & Gibt die Position des Zeichens \lstinline|c| in der \href{https://en.wikipedia.org/wiki/List_of_Unicode_characters}{Unicode-Tabelle} zurück    & \begin{lstlisting}[numbers=none]
ord('A') # 65
ord('B') # 66
ord('Z') # 90
\end{lstlisting}                                   \\\midrule
        \lstinline|chr(n)|                 & Gibt das Zeichen an der \lstinline|n|-ten Position der \href{https://en.wikipedia.org/wiki/List_of_Unicode_characters}{Unicode-Tabelle} zurück & \begin{lstlisting}[numbers=none]
chr(65) # "A"
chr(66) # "B"
chr(67) # "C"
\end{lstlisting}                                   \\\midrule\midrule
        \lstinline|text1.count(text2)|     & Zählt, wie oft der Text \lstinline|text2| im Text \lstinline|text1| vorkommt                                                                   & \begin{lstlisting}[numbers=none]
text = "Das Haus ist gross und das Dach ist grün."
n = text.count("ist") # 2
\end{lstlisting}                                   \\\midrule
        \lstinline|text1.find(text2)|      & Gibt den niedrigsten Index des Teiltexts \lstinline|text2| zurück, falls dieser vorhanden ist, ansonsten \lstinline|-1|                        &\begin{lstlisting}[numbers=none]
text = "Das Haus ist gross und das Dach ist grün."
text.find("ist") # 9
\end{lstlisting}\\\midrule
        \lstinline|text.replace(old, new)| & Ersetzt alle Vorkommen des Teil-Texts \lstinline|old| durch \lstinline|new|.                                                                   & \begin{lstlisting}[numbers=none]
text = "hallo"
text = text.replace("a", "e") # "hello"
\end{lstlisting}                                   \\\midrule\midrule
        \lstinline|text.split(sep)|        & Zerlegt den Text an jedem \texttt{sep} und gibt eine Liste der Teiltexte zurück.                                                               & \begin{lstlisting}[numbers=none]
text = "a,b,c"
liste_texte = text.split(",") 
# ["a", "b", "c"]
\end{lstlisting}                                   \\\midrule
        \lstinline|sep.join(liste)|        & Verbindet die Elemente von \texttt{liste} zu einem Text, getrennt durch den Text \texttt{sep}.                                                 & \begin{lstlisting}[numbers=none]
txt = ":".join(["a", "b", "c"]) 
# "a:b:c"
\end{lstlisting}                                   \\\midrule\midrule
        \lstinline|str(n)|                 & Zahl \texttt{n} in einen Text (engl. \textit{string}) umwandeln                                                                                & \begin{lstlisting}[numbers=none]
 zahl = 10
 zahl_als_text = str(zahl) # "10"
\end{lstlisting}                                   \\\midrule
        \lstinline|int(s)|                 & Text \texttt{s} in eine ganze Zahl (engl. \textit{integer}) umwandeln                                                                          & \begin{lstlisting}[numbers=none]
zahl_als_text = "10"
zahl = int(zahl_als_text) # 10
\end{lstlisting}
    \end{longtable}
...
\end{table}
\begin{myexercise}
    Schreiben Sie ein Programm, um jeden Buchstaben \enquote{e} oder \enquote{E} des folgenden Texts durch den Buchstaben \enquote{a} oder \enquote{A} zu ersetzen:
...
\begin{myexercise}
    Schreiben Sie ein Programm, um zu zählen, wie häufig der Buchstabe \enquote{e} oder \enquote{E} im gesamten folgenden Text vorkommt:
...
\end{lstlisting}
    ...
    \begin{myanswer}
        \begin{lstlisting}
my_text = "Eines Tages entschied der Elefant, einen edlen Teppich zu weben."
print(my_text.count("e") + my_text.count("E")) # 15
\end{lstlisting}
    \end{myanswer}
\end{myexercise}
...
\begin{myexercise}\label{ex:oneline-ascii-shift-char-safety}
    ...
    Tipp: Der grösste gültige Unicode-Wert für Grossbuchstaben ist 90 für den Buchstaben \enquote{\texttt{Z}}. Wenn man einen Wert bekommt, der 91 oder grösser ist, muss man ihn verkleinern!
