...
\subsubsection*{Schritt 1: Umgebung erstellen}
Dies erstellt einen versteckten, isolierten "`Sandbox"'-Ordner namens \texttt{.venv} in Ihrem Projektverzeichnis, in dem alle Bibliotheken gespeichert werden.
...
\subsubsection*{Schritt 2: Umgebung aktivieren}
Dies weist Ihren Computer an, diese spezifische Sandbox zu nutzen. \textbf{Wichtige Überprüfung:} Sie müssen \texttt{(.venv)} ganz am Anfang Ihrer Terminal-Befehlszeile sehen.
\begin{lstlisting}[language=bash]
# Mac / Linux:
...
# Windows (Eingabeaufforderung / CMD - Alternative):
.venv\Scripts\activate.bat
\end{lstlisting}

\subsubsection*{Schritt 3: Bibliotheken installieren}
Da Sie sich nun sicher innerhalb der virtuellen Umgebung befinden, können Sie die benötigten Kurs-Tools fehlerfrei installieren:
\begin{lstlisting}[language=bash]
...
% --- Windows Troubleshooting Box ---
\begin{tcolorbox}[title=\textbf{⚠️ Windows-Fehlerbehebung (Skript-Ausführungsfehler)}, fonttitle=\bfseries]
Falls Studierende unter Windows eine rote Fehlermeldung erhalten (z.\,B. \textit{"...kann nicht geladen werden, da die Ausführung von Skripten auf diesem System deaktiviert ist"}), muss lediglich dieser Befehl \textbf{einmalig} in der PowerShell ausgeführt und Schritt 2 wiederholt werden:
\begin{lstlisting}[language=bash, backgroundcolor=\color{white}, frame=none]
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope Process
\end{lstlisting}
\end{tcolorbox}

\subsubsection*{Schritt 4: VS Code starten und Python-File öffnen}
\begin{enumerate}
