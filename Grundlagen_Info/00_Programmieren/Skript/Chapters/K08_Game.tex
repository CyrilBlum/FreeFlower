...
\begin{lstlisting}
# 1. Highscore aus Textdatei laden (mit try/except, falls Datei noch nicht existiert)
try:
...
# 2. Neuen Highscore in Textdatei schreiben
with open("highscore.txt", "w") as file:
    file.write(str(highscore))
\end{lstlisting}
\end{mydefinition}

\begin{myexercise}[Pong-Spiel mit dauerhaftem Highscore]\label{myexercise:HighscoreTxt}
    Erstellen Sie das klassische Pong-Spiel mit zwei Paddles, einem Ball und dauerhaftem Highscore-Speicher in einer Datei \texttt{highscore.txt}.
    ...
    \begin{enumerate}
        ...
        \item Kollisionserkennung für Ball und Paddles, Richtung von Ball umkehren bei Kollision oder oberen/unteren Rändern
        \item Punkte zählen und aktuellen Spielstand im Fenster anzeigen
        \item \textbf{Highscore laden \& anzeigen:} Laden Sie vor der Game-Schleife den Highscore aus der Datei \texttt{highscore.txt}. Fangen Sie mit \lstinline|try / except FileNotFoundError:| den Fall ab, dass die Datei beim allerersten Start noch nicht existiert (in diesem Fall ist \lstinline|highscore = 0|). Zeichnen Sie den Highscore unterhalb des aktuellen Spielstands.
        \item \textbf{Highscore speichern:} Sobald der Punktestand eines Spielers den bisherigen Highscore übertrifft, aktualisieren Sie \lstinline|highscore| und schreiben den neuen Wert mit \lstinline|with open("highscore.txt", "w") as file:| direkt in die Datei.
...
\section{Objektorientierte Spielobjekte mit Pygame-Sprites}
