...
\begin{lstlisting}
# 1. Highscore aus Textdatei lesen
...
\begin{myexercise}[Pong-Erweiterung: Highscore speichern und anzeigen]\label{myexercise:HighscoreTxt}
    Erweitern Sie Ihr Pong-Spiel so, dass der bisherige Highscore dauerhaft in einer Textdatei gespeichert, beim Spielstart geladen und im Spielfenster angezeigt wird:
    \begin{enumerate}
        \item Erstellen Sie im selben Ordner wie Ihr Skript eine Datei namens \texttt{highscore.txt} und schreiben Sie eine \lstinline|0| hinein.
        \item \textbf{Highscore laden:} Öffnen und lesen Sie die Datei vor der Game-Schleife mit \lstinline|with open("highscore.txt", "r") as file:| und speichern Sie den Wert als Zahl in der Variable \lstinline|highscore|.
        \item \textbf{Highscore anzeigen:} Zeichnen Sie den Highscore mit \lstinline|font.render(...)| und \lstinline|screen.blit(...)| unterhalb des aktuellen Spielstands im Spielfenster.
