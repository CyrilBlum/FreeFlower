...
\begin{myexercise}
	\lstinputlisting[language=Python,caption=\texttt{fehlerscope.py},label=listing:scope]{Grundlagen_Info/00_Programmieren/Skript/Code/K05_Functions/fehlerscope.py}
	\begin{enumerate}
		\item Führen Sie das Programm aus.
		\item Weshalb entsteht der Fehler?
		\item Wie könnte man das Problem lösen?
	\end{enumerate}
	\begin{myanswer}
		\begin{enumerate}
			\item -
			\item Das Programm führt zu einer Fehlermeldung, da die Variable \lstinline|zaehler| innerhalb der Funktion \lstinline|erhoehe| nicht existiert. Obwohl es eine globale Variable mit dem Namen \lstinline|zaehler| gibt, ist diese innerhalb der Funktion nicht zugänglich.
			\item Man könnte das Problem lösen, indem man die Zeile \lstinline|zaehler = 0| in die Funktion \lstinline|erhoehe| verschiebt. Dann wird die Variable \lstinline|zaehler| aber ausserhalb der Funktion nicht mehr existieren, was wahrscheinlich nicht das gewünschte Verhalten ist.
		\end{enumerate}
	\end{myanswer}
\end{myexercise}

\begin{myexercise}
	\lstinputlisting[language=Python,caption=\texttt{scopeausgabevorhersagen.py},label=listing:scopeausgabevorhersagen]{Grundlagen_Info/00_Programmieren/Skript/Code/K05_Functions/scopeausgabevorhersagen.py}
	\begin{enumerate}
		\item \textbf{Beantworten Sie folgende Frage, bevor Sie das Programm ausführen:} Was glauben Sie, wird die Ausgabe des Programms sein?
		\item Welche Variable ist global, welche lokal?
	\end{enumerate}
	\begin{myanswer}
		\begin{enumerate}
			\item \begin{lstlisting}
				...
				100
			\end{lstlisting}
			\item \lstinline|x|: global (das \lstinline|x|, das in der Zeile \lstinline|x = 100| definiert wird; \lstinline|x| in der Funktion ist ein Parameter, also lokal), \lstinline|y|: lokal
		\end{enumerate}
	\end{myanswer}
\end{myexercise}
% \subsection{Weitere Aufgaben}
% \begin{myexercise}
% 	Schreiben Sie eine Funktion \lstinline|rahmen(text)|, welche einen Text in einem Rahmen aus Sternen ausgibt. Verwenden Sie dazu die Funktion \lstinline|len|, um die Länge des Textes zu bestimmen, und verwenden Sie diese Länge, um die Anzahl Sterne zu berechnen, welche für den Rahmen benötigt werden.

% 	\begin{myanswer}
% 		\lstinputlisting[language=Python,caption=\texttt{sternenrahmen.py},label=listing:quadratzahl]{Grundlagen_Info/00_Programmieren/Skript/Code/Further\ Exercises/sternenrahmen.py}
% 	\end{myanswer}
% \end{myexercise}
\newpage
...
\begin{myexercise}
	...
	\begin{myanswer}
		\lstinputlisting[language=Python,caption=\texttt{notenschnitt.py},label=listing:notenschnitt]{Grundlagen_Info/00_Programmieren/Skript/Code/K05_Functions/notenschnitt.py}
	\end{myanswer}
