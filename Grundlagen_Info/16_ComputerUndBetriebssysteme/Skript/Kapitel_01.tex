...
Bevor wir über Bits und Rechenregeln sprechen, starten wir bei der Hardware: Ein Chip ist zunächst ein Netzwerk aus leitenden Verbindungen und schaltenden Bauelementen. Die zentrale Idee lautet: \textbf{Elektrische Zustände werden als logische Zustände interpretiert}.
...
\section{Bits und Byte als Abstraktion}

Ein Computer verarbeitet Informationen in Form von \textbf{Bits}. Ein Bit (von \emph{binary digit}, dt. Binärstelle) kann genau zwei Zustände annehmen: \texttt{0} oder \texttt{1}. Die elektrische Realität (Spannung, Strom, Ladung) wird damit auf eine stabile mathematische Darstellung reduziert.
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{Gängige Einheiten für Datenmengen.}
    \label{tab:einheiten}
\end{table}

\begin{myexample}[Grösse von Dateien]
    Ein kurzes Textdokument benötigt wenige Kilobytes, ein Foto einige Megabytes und ein hochauflösender Film mehrere Gigabytes Speicherplatz.
\end{myexample}

\section{Vom Transistor zum Gatter}

In modernen Chips übernehmen \textbf{Transistoren} die Rolle extrem schneller Schalter. Ein logisches Gatter ist damit keine \enquote{magische Box}, sondern eine konkrete Schaltung aus vielen Transistoren.
...
\begin{mydefinition}[Hardware-relevante Kennwerte]
    ...
    \begin{itemize}
        \item \textbf{Propagationsverzögerung}: Zeit zwischen Eingangsänderung und stabiler Ausgangsänderung.
        \item \textbf{Leistungsaufnahme}: Energiebedarf durch Schalten und Leckströme.
        \item \textbf{Fan-out}: Anzahl von Eingängen, die ein Ausgang zuverlässig treiben kann.
    \end{itemize}
\end{mydefinition}

\section{Logische Grundverknüpfungen}

Die Grundlage aller Rechenoperationen eines Computers bilden \textbf{Boolesche Funktionen}: Funktionen, die einen oder mehrere Bits als Eingabe erhalten und genau ein Bit als Ausgabe liefern. Die drei elementaren Verknüpfungen sind \textbf{NOT}, \textbf{AND} und \textbf{OR}.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[circuit logic US, scale=1.4]
        ...
        \end{scope}
        \node[draw,rounded corners,fit=(notBB),inner sep=6pt] {};
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[circuit logic US, scale=1.4]
        ...
        \end{scope}
        \node[draw,circle,fit=(andBB),inner sep=6pt] {};
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[circuit logic US, scale=1.4]
        ...
        \end{scope}
        \node[draw,circle,fit=(orBB),inner sep=6pt] {};
    \end{tikzpicture}
...
\begin{mydefinition}[NAND-Gatter]
    Das \textbf{\gls{NAND}-Gatter} liefert die Negation des AND:
    \[
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[circuit logic US, scale=1.4]
        ...
        \end{scope}
        \node[draw,circle,fit=(nandBB),inner sep=6pt] {};
    \end{tikzpicture}
...
Das NAND-Gatter ist besonders bedeutsam: Durch Kombinationen von NAND-Gattern lassen sich \emph{alle} anderen logischen Funktionen (NOT, AND, OR, \ldots) nachbauen. Man sagt, NAND ist \textbf{funktional vollständig}. In vielen Fertigungsprozessen ist es technologisch günstig, aus wenigen standardisierten Gattertypen sehr grosse Schaltungen aufzubauen.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[circuit logic US, scale=1.3]
        ...
        \end{scope}
        \node[draw,circle,fit=(haBB),inner sep=8pt] {};
    \end{tikzpicture}
