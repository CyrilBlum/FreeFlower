\begin{tikzpicture}[
    ...
    \node[block, fill=ForestGreen!25] (alu) at (-2.8, 0) {ALU};
    \node[block, fill=Gray!20, minimum width=2.0cm] (reg) at (0, 0) {Register};
    \node[block, fill=RoyalBlue!20] (cw) at ( 2.8, 0) {Steuerwerk};

    \draw[<->] (alu) -- (reg);
    \draw[<->] (reg) -- (cw);

    % RAM
    \node[block, fill=Teal!25, minimum width=4.0cm] (ram) at (0, -2.5) {Arbeitsspeicher (RAM)};
