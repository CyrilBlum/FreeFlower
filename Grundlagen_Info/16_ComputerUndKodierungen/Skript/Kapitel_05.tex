...
\begin{figure}[H]
    \centering
    \input{Grundlagen_Info/16_ComputerUndKodierungen/Figures/tikz_software_stack.tex}
...
\begin{figure}[H]
    \centering
    \input{Grundlagen_Info/16_ComputerUndKodierungen/Figures/tikz_multitasking.tex}
...
\begin{figure}[H]
    \centering
    \input{Grundlagen_Info/16_ComputerUndKodierungen/Figures/tikz_tastendruck_ablauf.tex}
...
\begin{myexercise}\label{myexercise:software-stack-zuordnung}
    Ordnen Sie die fünf Elemente (A--E) der jeweils richtigen Schicht im Software-Stack zu:
    \begin{center}
        \input{Grundlagen_Info/16_ComputerUndKodierungen/Figures/tikz_software_stack_aufgabe.tex}
    \end{center}
    \begin{myanswer}
        \begin{center}
            \input{Grundlagen_Info/16_ComputerUndKodierungen/Figures/tikz_software_stack_loesung.tex}
        \end{center}
        \begin{itemize}
            \item \textbf{A (NTFS-Dateisystem)} $\to$ Betriebssystem (Kernel)
            \item \textbf{B (Google Chrome)} $\to$ Anwendungen
            \item \textbf{C (CPU führt Maschinenbefehl aus)} $\to$ Hardware
            \item \textbf{D (Grafikkartentreiber)} $\to$ Gerätetreiber
            \item \textbf{E (Aufruf \texttt{open("datei.txt")})} $\to$ Systemaufrufe / API
        \end{itemize}
    \end{myanswer}
\end{myexercise}
...
\begin{table}[H]
    \centering
    \begin{tabular}{lll}
        \textbf{Betriebssystem} & \textbf{Hersteller} & \textbf{Typisches Einsatzgebiet} \\\midrule
        Windows 11              & Microsoft           & PC, Laptop \\
        macOS                   & Apple               & Mac-Computer \\
        GNU+Linux (Ubuntu, Debian \ldots) & Open Source  & Server, PC, Laptop, eingebettete Systeme \\
