...
\section{Definition der Kolmogorov-Komplexität}
...
\par
Die Sprache \texttt{C'++} unterscheidet sich von Standard-\texttt{C++} darin, dass in einem \texttt{C'++}-Programm binäre Blöcke durch die Escape-Sequenzen \cppinline{***} und \cppinline{+++} markiert werden dürfen. Zwischen diesen Begrenzungszeichen steht stets ein binäres Wort $w$. Der Inhalt des Blocks besitzt für sich allein noch keine numerische Bedeutung. Die Schreibweise rund um den Block legt fest, wie das Programm seine Bits verwendet:
\begin{itemize}
	\item \texttt{\detokenize{"***w+++"}} verwendet den Block als binäres Wort. Insbesondere gibt \texttt{\detokenize{std::cout << "***w+++";}} die Bits von $w$ unverändert aus.
	\item \cppinline{0b***w+++} interpretiert den Block als Binärdarstellung einer natürlichen Zahl, also als den Wert $val_2(w)$. Beispielsweise hat \cppinline{0b***1010+++} den Zahlenwert $10$, während \texttt{\detokenize{"***1010+++"}} das vierstellige Wort $1010$ bezeichnet.
\end{itemize}
Die zweite Schreibweise erweitert die aus Standard-\texttt{C++} bekannte Notation \cppinline{0b1010} für Binärzahlen. Ausdrücke wie \cppinline{0b***w+++} gehören jedoch eigens zur Sprache \texttt{C'++} und sind kein Standard-\texttt{C++}. Das Präfix \cppinline{0b} gehört zum gewöhnlichen, eindeutig kodierten Programmteil und nicht zum binären Rohdatenblock.
\par
Ein Block darf nur in einer dieser beiden Formen auftreten. Kommen Escape-Sequenzen in einem Programm mehrfach vor, so muss zwischen ihnen stets dasselbe Wort $w$ stehen; andernfalls meldet \texttt{g'++} einen semantischen Fehler. Derselbe Block darf an einer Stelle als Wort und an einer anderen Stelle als Zahl verwendet werden.
\par
...
\par
In den folgenden Programmschemata stehen Ausdrücke wie \cppinline{0b***n+++} beziehungsweise \cppinline{0b***n**2+++} dafür, dass für jedes feste $n$ konkret die Binärdarstellung von $n$ beziehungsweise von $n^2$ zwischen den Begrenzungszeichen steht. Natürliche Zahlen werden dabei gemäss \cref{mydefinition:binunddec} ohne führende Nullen dargestellt; die Zahl $0$ wird durch den Block $0$ dargestellt. Die Ausdrücke \cppinline{n} und \cppinline{n**2} selbst sind also keine Bestandteile des kompilierten Programms. Für $n=10$ wird aus dem Schema \cppinline{0b***n+++} beispielsweise der konkrete Ausdruck \cppinline{0b***1010+++}.
...
\begin{myexercise}[Alle Bitmuster in einem Wort]\label{myexercise:definition_kolmogorov}
	Für $n\geq 1$ entsteht das Wort $w_n$, indem wir alle binären Wörter der Länge $n$ in aufsteigender Reihenfolge ihres Zahlenwerts aneinanderhängen. Führende Nullen werden dabei beibehalten. Beispielsweise ist $w_2=00\,01\,10\,11=00011011$.
	\begin{enumerate}
		\item Bestimme $w_3$ und leite eine Formel für die Länge von $w_n$ her.
		\item Entwirf einen Algorithmus, der $w_n$ generiert, ohne die einzelnen Teilwörter im Programm zu speichern. Pseudocode genügt. Welche Angabe muss für jedes $n$ im Programm hinterlegt werden, damit es ohne Eingabe genau $w_n$ ausgibt?
		\item Stelle dir vor, dein Algorithmus wird in \texttt{C'++} umgesetzt. Verwende für die von $n$ abhängige Angabe einen binären Rohdatenblock. Begründe damit eine obere Schranke für $C(w_n)$ der Form $\abs{bin(n)}+c$. Wofür steht $c$, und weshalb ist es unabhängig von $n$? Vergleiche für $n=1024$ die Wortlänge mit dieser Schranke.
		\item In $w_n$ kommt jedes Bitmuster der Länge $n$ vor. Bedeutet dies, dass $w_n$ einen grossen Informationsgehalt im Sinne der Kolmogorov-Komplexität haben muss? Begründe anhand deiner Ergebnisse.
	\end{enumerate}
	\begin{myanswer}
		\begin{enumerate}
			\item Es ist $w_3=000\,001\,010\,011\,100\,101\,110\,111$. Es gibt $2^n$ binäre Wörter der Länge $n$, also gilt $\abs{w_n}=n\cdot 2^n$.
			\item Eine mögliche Lösung lautet:
\begin{lstlisting}[language=Python]
n = ...  # Im Programm gespeicherter Wert, keine Eingabe
bits = [0] * n
while True:
    for bit in bits:
        print(bit, end="")
    ...
    bits[i] = 1
\end{lstlisting}
			Die Liste beginnt mit $n$ Nullen und wird nach jeder Ausgabe wie ein binärer Zähler um eins erhöht. Nach der Ausgabe von $n$ Einsen endet das Programm. Nur der Wert $n$ muss hinterlegt werden; alle Teilwörter werden daraus berechnet.
			\item In \texttt{C'++} kann $n$ durch das Schema \cppinline{0b***n+++} hinterlegt werden. Der Rohdatenblock benötigt $\abs{bin(n)}$ Bits. Die Konstante $c$ ist die Länge des übrigen kompilierten Programms einschliesslich des Verweises auf den Block. Dieser Programmteil enthält immer denselben Zähl- und Ausgabealgorithmus und hängt daher nicht von $n$ ab. Somit gilt
			\begin{align*}
				C(w_n)\leq\abs{bin(n)}+c.
			\end{align*}
			Für $n=1024$ hat das Wort $1024\cdot 2^{1024}$ Bits, während $bin(1024)=10000000000$ nur $11$ Bits hat. Also ist $C(w_{1024})\leq 11+c$. Der während der Ausführung benötigte Speicher für die Liste gehört nicht zur Programmlänge.
			\item Nein. Obwohl alle Bitmuster vorkommen, ist ihre Anordnung durch eine kurze Regel festgelegt. Das gesamte Wort lässt sich aus $n$ und einem festen Algorithmus rekonstruieren. Die Vielfalt der enthaltenen Teilwörter allein erzwingt daher keine hohe Kolmogorov-Komplexität.
		\end{enumerate}
	\end{myanswer}
\end{myexercise}

\section{Invarianz der Programmiersprache}\label{sec:Sprachinvarianz}
Die Länge eines Programms hängt natürlich von der verwendeten Programmiersprache ab. Trotzdem ist die Wahl der Sprache für die Kolmogorov-Komplexität nicht entscheidend. Der Grund dafür lässt sich mit einem Übersetzer erklären.

Nehmen wir an, ein Programm $p$ ist in einer Sprache $S$ geschrieben. Unsere Sprache $U$ kann dieses Programm ausführen, wenn wir ihr zusätzlich einen Übersetzer für $S$ mitgeben. Diesen festen Übersetzer bezeichnen wir mit $i_S$. Das zusammengesetzte Programm sieht also so aus:
\begin{align*}
	\underbrace{i_S}_{\text{Übersetzer für }S}\quad
	\underbrace{p}_{\text{eigentliches Programm}}.
\end{align*}
Der Übersetzer $i_S$ muss nur wissen, wie die Sprache $S$ funktioniert. Er ist daher für alle $S$-Programme derselbe. Insbesondere wird er nicht länger, wenn das Programm $p$ oder dessen Ausgabe länger wird.

Eine Sprache $U$, die auf diese Weise Programme aus allen anderen Programmiersprachen ausführen kann, nennen wir \textit{universell}. Wir nehmen an, dass unsere Sprache \texttt{C'++} eine solche Sprache ist.

\begin{mytheorem}[Invarianzsatz]\label{mytheorem:Sprachinvarianz}
	Es sei $U$ unsere universelle Sprache und $S$ eine andere Programmiersprache. Mit $C_U(w)$ beziehungsweise $C_S(w)$ bezeichnen wir die Kolmogorov-Komplexität von $w$, gemessen mit der jeweiligen Sprache. Dann gibt es eine feste Zahl $c_S$, sodass
	\begin{align*}
		C_U(w)\leq C_S(w)+c_S
	\end{align*}
	für alle binären Wörter $w$ gilt.
	\begin{myproof}
		Sei $p$ ein kürzestes $S$-Programm für $w$. Es hat die Länge $C_S(w)$. Fügen wir den festen Übersetzer $i_S$ hinzu, erhalten wir ein $U$-Programm für dasselbe Wort $w$. Dieses Programm hat die Länge
		\begin{align*}
			\abs{i_S}+C_S(w).
		\end{align*}
		Setzen wir $c_S:=\abs{i_S}$, folgt
		\begin{align*}
			C_U(w)\leq C_S(w)+c_S.
		\end{align*}
		Der zusätzliche Platz $c_S$ ist für alle Wörter $w$ gleich, weil der Übersetzer stets derselbe ist.
	\end{myproof}
\end{mytheorem}
Sind $U$ und $S$ beide solche universellen Sprachen, können sie sich gegenseitig übersetzen. Für die Übersetzung von $S$ nach $U$ benötigen wir wie oben den zusätzlichen Platz $c_S$. Für die umgekehrte Richtung, also von $U$ nach $S$, bezeichnen wir den zusätzlichen Platz mit $c_U$. Damit gilt für alle binären Wörter $w$:
\begin{align*}
	C_U(w)&\leq C_S(w)+c_S,\\
	C_S(w)&\leq C_U(w)+c_U.
\end{align*}
Die beiden Übersetzer können unterschiedlich lang sein; deshalb müssen $c_S$ und $c_U$ nicht gleich gross sein. Wählen wir das Maximum
\begin{align*}
	c_{U,S}:=\max\{c_S,c_U\},
\end{align*}
so deckt diese Konstante den zusätzlichen Platz in beiden Richtungen ab. Ihre Kolmogorov-Komplexitäten unterscheiden sich deshalb höchstens um diese feste Konstante:
\begin{align*}
	\abs{C_U(w)-C_S(w)}\leq c_{U,S}.
\end{align*}
Dabei hängt $c_{U,S}$ von den beiden Sprachen und den gewählten Übersetzern, aber nicht von $w$ ab. In diesem Sinne spielt die Wahl der Programmiersprache keine wesentliche Rolle.
...
\begin{myremark}[Denselben binären Block mehrfach verwenden]\label{myremark:bounded-repetitions}
	Betrachten wir ein festes Programmschema, in dem dasselbe binäre Wort $w$ an $k$ verschiedenen Stellen verwendet wird, wobei beispielsweise $1\leq k\leq 5$ gilt. Die Zahl $5$ ist dabei willkürlich gewählt und besitzt keine besondere Bedeutung. Entscheidend ist lediglich, dass die Anzahl der Vorkommen durch eine im Voraus festgelegte Konstante beschränkt ist. Anstelle von $5$ könnten wir jede andere feste obere Schranke $m$ wählen und $1\leq k\leq m$ voraussetzen.

	Die Vorkommen müssen weder im Quelltext nebeneinanderstehen noch muss die Ausgabe die Form $w^k$ besitzen. Beispielsweise darf das Programm die einzelnen Vorkommen von $w$ in verschiedenen Funktionen oder an verschiedenen Stellen der Ausgabe verwenden.

	In einem konkreten \texttt{C'++}-Programm werden alle diese Vorkommen durch \cppinline{***w+++} gekennzeichnet. Der Compiler \texttt{g'++} speichert nicht jedes Vorkommen separat. Er legt $w$ genau einmal als binären Rohdatenblock ab und ersetzt jedes der höchstens fünf Vorkommen durch einen Verweis auf diesen gemeinsamen Block. Der für $w$ benötigte Speicherplatz beträgt daher genau $\abs{w}$ Bits und nicht $k\abs{w}$ Bits.

	Ein einfaches Beispiel ist das folgende Programmschema:
	\begin{lstlisting}[language=C++,escapechar=!]
#include <iostream>
...
void use_w() {
	...
	\end{lstlisting}
	Dieses Programm erzeugt $x_w=w1w$. Die Ausgabe ist also im Allgemeinen nicht von der Form $w^k$, obwohl der Block $w$ an zwei verschiedenen Stellen des Programms verwendet wird.
	Nehmen wir nun an, dass ausser dem eingesetzten Block $w$ kein Teil des Programms von $w$ abhängt. Bezeichnen wir seine Ausgabe mit $x_w$, so existiert eine Konstante $c$ mit
	\begin{align*}
		C(x_w)\leq \abs{w}+c.
	\end{align*}
	Die Konstante $c$ umfasst den festen Programmcode und die höchstens fünf Verweise auf den gemeinsamen Datenblock. Weil deren Anzahl beschränkt ist, hängt $c$ nicht von $w$ ab. Für eine beliebige feste Schranke $m$ erhalten wir entsprechend eine Konstante $c_m$ mit
	\begin{align*}
		C(x_w)\leq \abs{w}+c_m.
	\end{align*}
	Die Konstante $c_m$ darf von der vorab gewählten Schranke $m$, aber nicht von $w$ abhängen. Auch wenn zwischen endlich vielen vorgegebenen Programmschemata gewählt werden darf, bleibt die Aussage richtig: Man wählt $c_m$ als den grössten konstanten Zusatzaufwand dieser Schemata.

	Die Voraussetzung einer festen oberen Schranke und eines festen Programmschemas ist wesentlich. Darf $k$ beliebig gross werden, kann der von $k$ abhängige Aufwand nicht ohne Weiteres in eine einzige, von $k$ unabhängige Konstante aufgenommen werden. Wird $k$ beispielsweise binär gespeichert und eine Schleife zur wiederholten Verwendung von $w$ eingesetzt, entsteht zusätzlich ein von der Beschreibungslänge von $k$ abhängiger Term, typischerweise von der Grössenordnung $O\lr{\log (k)}$. Falls weitere Programmteile oder die Positionen der Vorkommen durch zusätzliche, beliebig lange Daten festgelegt werden, müssen auch diese Daten in der Beschreibung gespeichert und zur Schranke addiert werden.

	Dasselbe Prinzip lässt sich mit einer Mehrband-Turingmaschine realisieren: Sie speichert $w$ einmal auf einem Arbeitsband und greift bei jeder benötigten Verwendung erneut auf dieses Band zu. Eine feste Mehrband-Turingmaschine kann in unserer optimalen universellen Beschreibungssprache durch einen festen Simulator beschrieben werden. Dessen Länge geht lediglich als additive Konstante in $c$ ein.
\end{myremark}
...
void Cn() {
    std::string wort(0b***!\colorbox{YellowGreen}{n}!+++, '1');
    std::cout << wort;
}
\end{lstlisting}
...
void Dn() {
    std::string wort;
    Natural M = 0b***!\colorbox{YellowGreen}{n}!+++;
    M = M * M;
    wort.reserve(M);
...
void Fn() {
    ...
    \end{lstlisting}
	Warum beweist dieses Programm sicherlich eine schlechtere obere Schranke für die Kolmogorov-Komplexität von $v_n$ als \cref{myexample:vn}? Tipp: Beachte dazu insbesondere \cref{myremark:bounded-repetitions}.
	\begin{myanswer}
		Die beiden Vorkommen von \cppinline{0b***n**2+++} bezeichnen dieselbe Binärdarstellung von $n^2$. Der Compiler \texttt{g'++} speichert den Block zwischen den Escape-Sequenzen deshalb nur einmal (siehe \cref{myremark:bounded-repetitions}). Damit liefert \texttt{Fn} die obere Schranke
		\begin{align*}
...
\begin{mydefinition}[$c$-inkompressibles binäres Wort]\label{mydefinition:cincompressible}
\leavevmode
...
\begin{mydefinition}[Zufällige binäre Wörter und Zahlen]\label{mydefinition:zufall}
\leavevmode
Ein binäres Wort $w$ heisst zufällig, falls $C(w)\geq\abs{w}$ gilt, wenn $w$ also $0$-inkompressibel ist. Eine natürliche Zahl $n>0$ heisst zufällig, falls ihr binärer Code zufällig ist, also $C(n)\geq\abs{bin(n)}$.
\end{mydefinition}

\begin{myexercise}
	\leavevmode
	In gewisser Literatur wird die Zufälligkeit einer natürlichen Zahl $n>0$ mit der Bedingung $C(n)\geq\abs{bin(n)} \textcolor{Red}{- 1}$ definiert anstelle von $C(n)\geq\abs{bin(n)}$. Wieso könnte es sinnvoll sein, diesen Term $-1$ in der Definition zuzulassen? Tipp: Überlege, was die Binärdarstellungen aller natürlichen Zahlen $n>0$ gemeinsam haben.
	\begin{myanswer}
		Da jede natürliche Zahl $n>0$ in binärer Darstellung mit einer $1$ beginnt, kann man die führende $1$ bereits als festen Bestandteil der Beschreibung betrachten. Die führende $1$ muss also nicht in der Beschreibung enthalten sein. Damit kann man die Länge der Beschreibung um $1$ Bit reduzieren, was den Term $-1$ in der Definition rechtfertigt. Gemäss dieser Defintion wird eine natürliche Zahl $n>0$ also erst dann als zufällig bezeichnet, wenn sie nicht kürzer als mit $\abs{bin(n)}-1$ beschrieben werden kann.
	\end{myanswer}
\end{myexercise}
