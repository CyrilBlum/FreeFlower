...
\begin{mytheorem}[Invarianzsatz]\label{mytheorem:Sprachinvarianz}
	...
	\end{myproof}
\end{mytheorem}
Sind $U$ und $V$ beide solche universellen Sprachen, können sie sich gegenseitig übersetzen. Ihre Kolmogorov-Komplexitäten unterscheiden sich deshalb höchstens um eine feste Konstante:
\begin{align*}
	\abs{C_U(w)-C_V(w)}\leq c_{U,V}
\end{align*}
Dabei hängt $c_{U,V}$ von den beiden Sprachen, aber nicht von $w$ ab. In diesem Sinne spielt die Wahl der Programmiersprache keine wesentliche Rolle.
...
\begin{myexample}[Denselben binären Block mehrfach verwenden]\label{myexample:bounded-repetitions}
...
void Fn() {
	...
	\begin{myanswer}
		Die beiden Vorkommen von \cppinline{0b***n**2+++} bezeichnen dieselbe Binärdarstellung von $n^2$. Der Compiler \texttt{g'++} speichert den Block zwischen den Escape-Sequenzen deshalb nur einmal. Damit liefert \texttt{Fn} die obere Schranke
		\begin{align*}
