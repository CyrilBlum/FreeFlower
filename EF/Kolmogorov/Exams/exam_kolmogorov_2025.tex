...
\end{solution}
	...
	\question[2] Sei $w_n := 0^{2^{n^3}}$ für jedes $n\in\N, n>0$ ein binäres Wort.	Gib eine (asymptotisch) möglichst gute obere Schranke für die Kolmogorov-Komplexität $C(w_n)$ von $w_n$ an, gemessen in der Länge $\abs{w_n}$ von $w_n$.
	\begin{solution}
...
\end{lstlisting}
		...
		Damit existieren Konstanten $c_0$ und $c_1$, sodass
\begin{align*}
	C(w_n) &\leq c_0 + \ceil{\log_2(n+1)} \leq \log_2(n) + c_1 = \log_2\lr{\sqrt[3]{\log_2\lr{\abs{w_n}}}} + c_1 = \\
	&=\log_2\lr{\log_2\lr{\abs{w_n}}} / 3 + c_1
\end{align*}
für beliebiges natürliches $n>0$.
	\end{solution}

	\question[2] Gib eine Funktion $f: \N\to\N$ an, sodass für jede natürliche Zahl $n$ mindestens drei Viertel der natürlichen Zahlen kleiner als $f(n)$ eine Kolmogorov-Komplexität von mindestens $2n$ haben.
    ...
    \end{itemize}
	\begin{solution}
		Die Anzahl aller Maschinencodes der Länge kleiner als $2n$ ist
		\begin{align*}
			\sum_{k = 0}^{2n-1}\lr{2^k} = 2^{2n} - 1.
		\end{align*}
		Es gibt also maximal $2^{2n} - 1$ binäre Wörter mit einer Kolmogorov-Komplexität kleiner als $2n$. Da alle natürlichen Zahlen paarweise verschiedene Binärdarstellungen haben, gibt es höchstens $2^{2n} - 1$ natürliche Zahlen mit einer Kolmogorov-Komplexität kleiner als $2n$.

		\vspace{\baselineskip}
		Damit ist die Wahl $f(n):= 4 \cdot 2^{2n}$ sinnvoll, denn $2^{2n} - 1 < 2^{2n} = \frac{f(n)}{4}$ und somit haben mehr als $3/4$ der natürlichen Zahlen kleiner als $f(n)$ eine Kolmogorov-Komplexität von mindestens $2n$.
	\end{solution}
	...
	\end{center}
    Erkläre die zentralen Ideen dieses Beweises in deinen eigenen (präzisen) Worten.
	\begin{solution}
		Siehe das Theorem zur Nichtberechenbarkeit der Kolmogorov-Komplexität im Skript.
	\end{solution}
	...
	\begin{align*}
		C\lr{y_i} \leq \ceil{\log_2\lr{\log_2\lr{\log_2\lr{\sqrt[4]{y_i}}}}} + c
	\end{align*}
	gilt.
	\begin{solution}
		...
		\end{align*}
			für alle $i\in\N, i>0$. Offensichtlich ist diese Zahlenfolge streng monoton wachsend und $i = \log_2\lr{\log_2\lr{\sqrt[4]{y_i}}}$.
...
\end{lstlisting}
		Da alle Programme \texttt{yi} bis auf die Binärdarstellung von $i$ gleich sind, folgt für geeignete Konstanten $c_0,c$:
	\begin{align*}
		C\lr{y_i} &\leq \ceil{\log_2\lr{i+1}} + c_0\\
		&\leq \ceil{\log_2\lr{\log_2\lr{\log_2\lr{\sqrt[4]{y_i}}}}} + c.
	\end{align*}
	\end{solution}
\end{questions}
