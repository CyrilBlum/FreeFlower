...
% Mini-Übung
\begin{frame}{Mini-Übung (im Computerraum)}
    \begin{myExBox}[title=Mini-Übung: Repository anlegen und erste Schritte]
        Legen Sie in GitHub Desktop ein neues Repository an (\url{https://github.com}), erstellen Sie die Datei \texttt{ideen.md} in VS Code, führen Sie einen Commit mit kurzer Nachricht aus und pushen Sie zum Remote. Öffnen Sie anschliessend den Verlauf (History) und betrachten Sie die Differenzen.
        \tcblower
        \textbf{Hinweis}: Verwenden Sie GitHub Desktop; die GUI zeigt History und Diff sichtbar.
    \end{myExBox}
\end{frame}

\begin{frame}[fragile]{Technische Aspekte}
    \begin{myExBox}[title=\emoji{key} SSH-Keys für Passwortlose Authentifizierung]
    \begin{enumerate}
        \item SSH-Key generieren: 
        \begin{lstlisting}[style=bash]
            ssh-keygen -t ed25519 -C "your@email.com"
        \end{lstlisting}
        \item SSH-Key zu GitHub hinzufügen: Kopieren Sie den Inhalt von 
        \begin{lstlisting}[style=bash]
            cat ~/.ssh/id_ed25519.pub
        \end{lstlisting}
        und fügen Sie ihn in GitHub unter \texttt{Settings > SSH and GPG keys} hinzu.
        \item Testen: Versuchen Sie, ein Repository zu klonen oder zu pushen, um die Verbindung zu testen. Sie sollten keine Passwortabfrage mehr erhalten.
    \end{enumerate}
    \end{myExBox}
...
\begin{frame}{Todo-Liste / Task-Umgebung}
    \centering
    \begin{tikzpicture}[every node/.style={align=center}]
        \node[draw, rounded corners, fill=blue!6] (t1) {`TODO.md` im Repo};
        \node[draw, rounded corners, fill=green!6, right=of t1] (t2) {GitHub Issues / Projects};
        \node[draw, rounded corners, fill=orange!6, below=of $(t1)!0.5!(t2)$] (t3) {Kanban / Week Planning Layer};
        \draw[->] (t1) -- (t2);
        \draw[->] (t2) -- (t3);
    \end{tikzpicture}
    \vspace{0.6em}
    \begin{block}{Empfehlung}
        Einfache `TODO.md` für persönliche Tasks, Issues für kollaboratives Tracking, Kanban/Calendar-Layer für Zeitplanung.
    \end{block}
\end{frame}

\begin{frame}{Konkrete Todo-Liste}
    \begin{myExBox}[title=Konkrete Todo-Liste]
        Repository initialisieren und \texttt{README.md} anlegen\\
        Ordnerstruktur erstellen (\texttt{dokumentation/}, \texttt{daten/}, \texttt{code/})\\
        \texttt{TODO.md} mit ersten Aufgaben anlegen\\
        \texttt{Week Planning}-Layer im Kalender anlegen (wöchentliche Tasks)\\
        Erste Sitzung: Recherche (Datum eintragen)
        \tcblower
        \textbf{Ziel}: Erste Struktur, erste Termine und erstes Tracking.
    \end{myExBox}
\end{frame}

% Abschlussfolie
\begin{frame}{Was bewusst weglassen wird}
    \centering
    \begin{block}{Ausklammern in dieser Doppellektion}
        Branches im Detail, Merge-Konflikte tief, Rebase, Pull-Request-Workflow, Git-Flow und ausführliche Kommandozeilen-Tricks.
    \end{block}
\end{frame}
