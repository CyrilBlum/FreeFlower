...
\begin{frame}[fragile]{Lernziele}
    \begin{itemize}[<+->]
        \item[\faSquare] Sie können Ihre Maturitätsarbeit in Teilaufgaben zerlegen
        \item[\faSquare] Sie können die Zeitplanung Ihrer Maturaarbeit mit einem GANTT-Chart visualisieren
        \item[\faSquare] Sie können einen Kalender-Layer für die Wochenplanung anlegen
        \item[\faSquare] Sie können ein einfaches Arbeitsjournal führen
        \item[\faSquare] Sie können eine sinnvolle Ordnerstruktur und Dokumentation anlegen
        \item[\faSquare] Sie verstehen den Unterschied zwischen Backup und Versionierung
        \item[\faSquare] Sie verstehen, warum Versionierung wichtig ist
        \item[\faSquare] Sie kennen einfache Git-Grundkonzepte und bewährte Arbeitsweisen mit Git, GitHub und VSCode
    \end{itemize}
\end{frame}


% Projektorganisation allgemein
\begin{frame}{Projektorganisation allgemein}{Zerlegung des Problems in grobe Schritte mit GANTT-Chart}
...
\begin{frame}
    \begin{myExBox}[title=Übung: GANTT-Chart für die Maturaarbeit erstellen]
        Machen wir später mit GitHub Projects!
    \end{myExBox}
\end{frame}
...
\begin{frame}{Projektorganisation allgemein}{Ordnerstruktur und Dokumentation}
    \centering
...
\begin{frame}[fragile]{Forschungsjournal}
    \small

    Ein Forschungsjournal hält Versuche, Parameter und Ergebnisse fest. So sehen Sie schnell, was funktioniert hat, was nicht und warum.\pause

    \begin{table}[H]
        \centering
        \begin{tabular}{p{2.7cm} p{4.0cm} p{3.3cm}}
            \small
            \textbf{Datum} & \textbf{Versuch / Einstellung}                   & \textbf{Ergebnis}                         \\
            \midrule\midrule
            2026-05-12     & Versuch A durchgeführt. Temperatur 40 $^\circ$C. & Ergebnis unbrauchbar.                     \\\midrule\pause
            2026-05-15     & Versuch wiederholt. Temperatur 60 $^\circ$C.     & Ergebnis deutlich besser.                 \\\midrule\pause
            2026-05-20     & Versuch mit 80 $^\circ$C durchgeführt.           & Ergebnis schlechter als bei 60 $^\circ$C. \\
        \end{tabular}
    \end{table}\pause
    \textbf{Empfohlene Tools}: Markdown-Dateien (in VS Code), physisches Notizbuch, OneNote, Word-Dokument.\\
    \url{https://cyrilblum.github.io/FreeFlower/markdown-tutorial.html}\\[.5cm]
    \resizebox{1cm}{!}{\qrcode{https://cyrilblum.github.io/FreeFlower/markdown-tutorial.html}}
\end{frame}


% Versionierung
\begin{frame}{Versionierung}{Das Problem}
    \centering
...
\begin{frame}[fragile]{Backup $\neq$ Versionierung}
    ...
    \small
    \begin{tabular}{p{2.7cm} c c p{4.4cm}}
        \textbf{Situation}            & \textbf{Backup} & \textbf{Git} & \textbf{Kurz gesagt}                                                 \\
        \hline
        Festplatte kaputt             & \cmark          & \xmark       & Backup rettet die Daten auf einem anderen Medium.                    \\
        Datei versehentlich gelöscht  & \cmark          & teilweise    & Backup hilft, Git hilft nur, wenn die Datei bereits versioniert war. \\
        Fehler vor 2 Wochen eingebaut & \xmark          & \cmark       & Git bringt Sie gezielt auf einen früheren Stand zurück.              \\
        Experiment rückgängig machen  & \xmark          & \cmark       & Git erlaubt Ausprobieren ohne Angst vor dem Rückweg.                 \\
    \end{tabular}

    \vspace{0.8em}
    \begin{block}{Merksatz}
        Backups schützen vor Datenverlust. Git schützt vor Fehlentscheidungen im Verlauf der Arbeit.
    \end{block}
\end{frame}
...
\begin{frame}{Repo-Struktur}{Spezielle Dateien}

    \begin{figure}
        \centering
        \begin{tikzpicture}[level distance=8mm, sibling distance=18mm, every node/.style={font=\small}]
            \node[draw, rectangle, rounded corners, fill=yellow!6] (root) {Maturitätsarbeit/}
            child { node[draw, rectangle, fill=green!6] {Dokumentation/} }
            child { node[draw, rectangle, fill=green!6, yshift=-7mm] {Daten/} }
            child { node[draw, rectangle, fill=green!6] {Bilder/} }
            child { node[draw, rectangle, fill=green!6, yshift=-7mm] {Code/} }
    ...
    \begin{center}
        \begin{tabular}{ r|l }
            \texttt{README.md} & Projektbeschreibung, Anleitungen \\
            \texttt{LICENSE} & Rechte und Nutzung \\
            \texttt{.gitignore} & Dateien, die nicht versioniert werden sollen \\
        \end{tabular}
    \end{center}
    \begin{center}
        Beispiel: \url{https://github.com/CyrilBlum/FreeFlower}
...
\begin{frame}{Tipps für die Maturaarbeit}
    ...
    \begin{block}{Kurz und praktisch}
        \begin{itemize}[<+->]
            \item Zu jedem Meilenstein ein kurzes, aussagekräftiges Commit.
            \item Commit-Nachrichten können auch zur Dokumentation im Arbeitsjournal verwendet werden.
            \item Branches für Experimente.
            \item Aufbau und Data-Handling in `\texttt{Dokumentation/}` dokumentieren.
            \item Grosse Daten ausserhalb des Repos sichern und verlinken und mit `\texttt{.gitignore}` ignorieren.
        \end{itemize}
    \end{block}
\end{frame}
